...
### Chapters 5–6: Line-by-Line Audit Against the PDF (2026-08-23)

The same page-by-page audit (chapter 5 against PDF pp.113–148, chapter 6 against PDF pp.149–174, using `scripts/qa/pdf_columns.py`) was carried out for chapters 5 and 6.

**Margin pull-quotes.** All margin-column epigraphs in both chapters were checked against the PDF and found already correctly wrapped in standalone `:::{aside}` blocks, with matching text and no nested-aside merging: chapter 5 (Sadi Carnot, Rudolf Clausius ×2, François-Marie Guyonneau de Pambour ×4 — §5.1.2 p.115, §5.1.4 p.118, §5.2.1 p.119 and p.120, §5.2.3 p.124, §5.4.2 p.131, §5.5.3 p.138) and chapter 6 (Sadi Carnot — §6.2.4 p.162). No changes needed.

**Stray enumerator-label fragments.** As in chapters 1–4, several answer blocks had a leftover fragment of the bold `**N.M**` exercise-number label sitting as its own line at the top of the answer, debris from the label being split across the extraction boundary: chapter 5 exercises 5.10 (`10**`) and 5.11 (`11**`); chapter 6 exercises 6.8 (`.8**`) and 6.9 (`.9**`). All removed.

**Missing exercise continuation (chapter 6, exercise 6.10).** Exercise 6.10 (*Industrial Refrigeration*, PDF p.168) ends its problem statement with "...pays 0.15 AC per kWh on average for electricity."; the final question of the problem ("What would be the annual financial savings generated by changing the refrigerator model?") had been left as a floating, unlabeled paragraph between exercise 6.10's answer and exercise 6.11, rather than inside exercise 6.10's own problem statement, ahead of its answer. Moved back into place.

**Stray footer boilerplate interleaved into body text.** As in chapter 3, three more instances of a stray `*Engineering Thermodynamics* by Olivier Cleynen` page-footer artifact were found spliced into running text: chapter 5, Example 5.5's solution (§5.2.4 p.126, after "...the saturation temperature is 133.52°C."); chapter 5, exercise 5.13's answer (PDF p.147, after "$W_{turbine}=-71\ MW$."); and chapter 6, exercise 6.7's answer (PDF p.166), where the footer text had also split the hyphenated word "refrig-erator" across a line break. All three removed, and the chapter 6 word rejoined as "refrigerator".

No other content-loss, garbling, or orphaned-artifact defects were found in chapters 5–6; `scripts/verify_book.py`'s structural exercise-count check continues to pass for both chapters after these fixes.

### Chapters 7–8: Line-by-Line Audit Against the PDF (2026-08-23)

The same page-by-page audit (chapter 7 against PDF pp.176–207, chapter 8 against PDF pp.208–239, using `scripts/qa/pdf_columns.py`) was carried out for chapters 7 and 8.

**Margin pull-quotes.** All margin-column epigraphs in both chapters were checked against the PDF and found already correctly wrapped in standalone `:::{aside}` blocks, with matching text, correct attributions, and no nested-aside merging: chapter 7 (Sadi Carnot ×3, Rudolf Clausius, Émile Clapeyron, William Thomson ×2, Rudolf Diesel — §7.2.1 p.179, §7.3.1 p.182, §7.3.2 p.183, §7.3.3 p.184, §7.4.4 p.192 and p.194, §7.5.1 p.195 ×2) and chapter 8 (Rudolf Clausius ×4, Sadi Carnot, Richard Feynman, William Thomson — §8.2.2 p.212, §8.3.2 p.217, §8.4.1 p.224, §8.4.1 p.225, §8.4.3 p.226, §8.5.3 p.230 ×2, §8.5.4 p.232). No changes needed.

**Orphaned equation-bound artifacts (chapter 8, §8.2.2).** The integral-sign superscript/subscript bounds "A" and "B" (from $\int_{\mathrm{A}}^{\mathrm{B}}$) had been stranded as bare single-letter lines around three consecutive equations defining entropy change (the unlabeled equation before 8/2, and equations 8/7 and 8/8), leaving the equations themselves missing their bounds entirely (rendering as plain $\int(\ldots)$). Restored the `\int _{\mathrm{A}}^{\mathrm{B}}` bounds inside each equation and removed the orphaned "B" / "A B" / "A" lines around them.

**Stray enumerator-label fragments.** As in chapters 1–6, several answer blocks had a leftover fragment of the bold `**N.M**` exercise-number label sitting as its own line at the top of the answer, debris from the label being split across the extraction boundary: chapter 7 exercises 7.8 (`.8**`) and 7.10 (`10**`); chapter 8 exercises 8.11 (`11**`) and 8.13 (`13**`). All removed.

**Stray footer boilerplate inside an answer.** Three more instances of a stray `*Engineering Thermodynamics* by Olivier Cleynen` page-footer artifact were found spliced into running text: chapter 7, exercise 7.10's answer (PDF p.205); and chapter 8, between example 8.2's solution and example 8.3's problem statement (§8.3.3 p.219), and again in exercise 8.14's answer (PDF p.239). None of the three instances split a word across a line break. All removed.

**Truncated figure alt text (chapter 8, figure 8.15).** The `:alt:` text for figure 8.15 (a public air-pump photo, exercise 8.11) was cut off mid-sentence ("...ensures the temperatures calcula"), while the figure's own caption text immediately below it carried the complete sentence. Restored the alt text to match the full caption ("...ensures the temperatures calculated in this problem are never attained.").

No other content-loss, garbling, or orphaned-artifact defects were found in chapters 7–8; `scripts/verify_book.py`'s structural exercise-count check continues to pass for both chapters after these fixes.

### Chapters 9–10: Line-by-Line Audit Against the PDF (2026-08-23)

The same page-by-page audit (chapter 9 against PDF pp.239–266, chapter 10 against PDF pp.267–302, using `scripts/qa/pdf_columns.py`) was carried out for chapters 9 and 10, the last chapter pair before the appendices.

**Margin pull-quotes.** All margin-column epigraphs in both chapters were checked against the PDF and found already correctly wrapped in standalone `:::{aside}` blocks, with matching text, correct attributions, and no nested-aside merging: chapter 9 (Sadi Carnot, François-Marie Guyonneau de Pambour, and — outside the usual historical-quote pattern — a first-person narrative aside by Jacques Darolles, 1998 — §9.1 p.241, §9.3.1 p.243, §9.5 p.263) and chapter 10 (Rudolf Diesel, Sadi Carnot, Louis Joseph Gay-Lussac, Aurel Stodola — §10.3.3 p.274, §10.4.1 p.278, §10.4.4 p.282, §10.5.2 p.285). No changes needed.

**Orphaned equation-bound artifacts (chapter 9, §9.3.1).** As in chapter 8, the integral-sign bounds "A" and "B" (from $\int_{\mathrm{A}}^{\mathrm{B}}$) had been stranded as bare single-letter lines around two equations (the restatement of equation 3/22, and equation 9/4 for reversible-pump work), leaving both equations missing their bounds. Restored the `\int _{\mathrm{A}}^{\mathrm{B}}` bounds inside each equation and removed the orphaned "B" / "A" lines, rejoining the sentences that continued after them.

**Split equation (chapter 10, equation 10/3) and mangled fraction (equation 10/4).** The compression-ratio definition $\varepsilon \equiv v_{\mathrm{A}}/v_{\mathrm{B}}$ (10/3) had been split into two separate, consecutive `{math}` blocks — one holding the numerator, the other the bare denominator — instead of a single fraction. Merged into one block as $\varepsilon \equiv \frac{v_{\mathrm{A}}}{v_{\mathrm{B}}}$. Immediately after, the Otto-cycle efficiency equation 10/4 had lost its fraction bar entirely, rendering as $\eta_{\mathrm{Otto}} = 1 - \varepsilon^{\gamma-1}1$ (a stray "1" tacked onto the exponent instead of forming $1/\varepsilon^{\gamma-1}$). Corrected to $\eta_{\mathrm{Otto}} = 1 - \frac{1}{\varepsilon^{\gamma-1}}$, matching the PDF's margin-column rendering of the same relation.

**Stray enumerator-label fragments.** As in chapters 1–8, three answer blocks in chapter 10 had a leftover fragment of the bold `**N.M**` exercise-number label sitting as its own line at the top of the answer, debris from the label being split during column reconstruction: exercises 10.3 (`.3**`), 10.5 (`.5**`), and 10.6 (`.6**`). All removed.

**Stray footer/appendix boilerplate spliced into running text.** Four more instances of the recurring page-footer artifact were found:
- Chapter 9, exercise 9.3's answer (PDF p.266) — a stray `*Engineering Thermodynamics* by Olivier Cleynen` line spliced in mid-answer. Removed.
- Chapter 10, after equation 10/10 (PDF p.282, §10.4.4) — the same footer line spliced between the equation and figure 10.14. Removed.
- Chapter 10, exercise 10.6's answer (PDF p.301–304) — the worst instance found in this pass: the footer line, a stray page number ("303"), the entire front-matter table of contents for the Appendix section (Appendix A1–A11, PDF p.303), a second stray page number ("304"), and the footer line again, had all been spliced onto the end of the answer, after "...an interesting compromise!". None of this belongs inside chapter 10 — it is chrome and next-section front matter from the two blank/transition pages (PDF pp.302–304) between chapter 10 and Appendix A1. All removed, restoring the answer's proper ending.

**Ligature/OCR-style word garbling in running text.** Several words in chapter 9 and chapter 10 body prose (not figure captions) were garbled versions of the correct PDF word, of the same "fi"/"fl"-ligature-transposition kind documented in `scripts/qa/fix_ligatures.py`, but in forms its regexes did not catch (an inserted space, or a swapped letter, rather than a same-word transposition): "fin ally" → "finally" (ch.9 §9.4.4); "ef-fi ciency" → "efficiency" (ch.9 exercise 9.1); "fin ite" → "finite", "ffity" → "fifty", "fti tightly" → "fit tightly", "briefly )" → "briefly)" (ch.10 §10.2.1/§10.3/§10.3.1); "inful ence" → "influence" ×2, ```orifcie``` → "orifice" (ch.10 §10.4.2/§10.4.4). Each was confirmed against `doc.get_text()` on the source PDF page before correcting. A pre-existing PDF-native typo ("the the painful reality", example 9.2; "air a as a working fluid", chapter 10 introduction; "to the the fact", eq. 10/4 discussion) was confirmed present in the raw PDF text itself and left untouched, as it is a source-document error rather than a conversion artifact.

No other content-loss, garbling, or orphaned-artifact defects were found in chapters 9–10; `scripts/verify_book.py`'s structural exercise-count check continues to pass for both chapters after these fixes.
