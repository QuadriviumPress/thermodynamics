...
**2.3 The First Law in a Closed System** The first law states that energy is indestructible (§1.1.2). If we supply $100 J$ of work to a closed system and it rejects $80 J$ in the form of heat, then “its” energy has increased by $20 J$. We call this increase the *change of internal energy*, $\Delta U$. In the form of an equation, the first law in a closed system is expressed by « $^{\mathrm{Let} \mathrm{therefore} \mathrm{be} Q \mathrm{the} \mathrm{whole}}$ quantity of heat which one must imthe equation: part to a body while it transitions on $Q_{1\rightarrow 2}+ W_{1\rightarrow 2}= \Delta U (2/1) ^{\mathrm{a} \mathrm{certain} \mathrm{path} \mathrm{from} \mathrm{one} \mathrm{state} \mathrm{into} \mathrm{an}-}$ other (whereby a withdrawn quantity for a stationary closed system; of heat is counted as a negatively imwhere $\Delta U = U_{2}- U_{1}$ is the change of internal energy $(J)$, parted heat quantity), so we divide this into three parts, of which the first com- $W_{1\rightarrow 2}$ is the work received by the system $(J)$, prehends the increase of the heat actuand $Q_{1\rightarrow 2}$ is the heat received by the system $(J)$. ally present in the body, the second the Unfortunately, internal energy $U$ is sometimes very difficult to measure. We $^{\mathrm{heat} \mathrm{consumed} \mathrm{for} \mathrm{internal} \mathrm{work} \mathrm{and}}$ the third the heat consumed for extershall see in chapters 4 and 5 that bodies store this internal energy in different $_{\mathrm{nal} \mathrm{work}. \mathrm{Of} \mathrm{the} \mathrm{first} \mathrm{part}, \mathrm{the} \mathrm{same}}$ holds true as has already been said of ways, and that it is intimately related to temperature. By definition, internal the second, that it is independent of energy $U$ is always positive, but its variation $\Delta U$ can also be negative. the manner in which the change has occurred, and therefore we can repreEquation 2/1 can be expressed with specific quantities: $^{\mathrm{sent} \mathrm{both} \mathrm{parts} \mathrm{together} \mathrm{by} \mathrm{a} \mathrm{function}} U$, of which, even if we do not yet know it more closely, we know at least this $m (q_{1\rightarrow 2}+ w_{1\rightarrow 2}) = m \Delta u$ much beforehand, that it is fully determined by the initial and final state of $q_{1\rightarrow 2}+ w_{1\rightarrow 2}= \Delta u$ (2/2) the body. » Rudolf Clausius, 1854 for a stationary closed system; *Über eine veränderte Form des* where $\Delta u = u_{2}- u_{1}$ is the change of specific internal energy $(J kg^{-1}), _{\mathrm{zweiten} \mathrm{Hauptsatzes} \mathrm{der}} w_{1\rightarrow 2}$ is the specific work received by the system $(J kg^{-1}), ^{\mathrm{mechanischen} \mathrm{Wärmetheorie} [13]}$ and $q_{1\rightarrow 2}$ is the specific heat received by the system $(J kg^{-1})$. We can rewrite this equation 2/2 to express it in its *differential form*: δ$q +$ δ$w = du$ for a stationary closed system; where $du$ is the infinitesimal change of specific internal energy $(J kg^{-1})$, δ$w$ is the (specific) infinitesimal work transfer $(J kg^{-1})$, and δ$q$ is the (specific) infinitesimal heat transfer $(J kg^{-1})$. In this equation 2/3, the mathematical operators $d$ and δ have slightly different meanings: $du$, an *exact differential*, represents an infinitesimal *change* that will integrate to $\Delta u = u_{2}-u_{1}$; on the other hand, an δ$w$, *inexact differential*, represents an infinitesimal *transfer* that will integrate to $w_{1\rightarrow 2}$. This distinction is further elaborated in Appendix A4 p. 316. When a fluid is brought back to its initial state (same pressure, same volume, same temperature), then it contains exactly the same amount of internal energy as before. The total energy it has received (in the form of heat or work) has therefore necessarily been returned to the surroundings in one form or another. We express this statement as follows: $Q_{\mathrm{cycle}}+ W_{\mathrm{cycle}}= 0$ for a complete thermodynamic cycle, where $W_{\mathrm{cycle}}$ is the work received by the system $(J)$, and $Q_{\mathrm{cycle}}$ is the heat received by the system $(J)$. This equation 2/4 is the reason why the first law is often stated—without adding much to our simple statement in chapter 1 — in the following way: “When a system has completed a full thermodynamic cycle, the algebraic sum of the heat it has supplied and the work it has done is zero.”
...
:::{math}
• the kilogram - force per square centimeter , which is almost equal
:::
...
becomes “stifefr”, and the pressure increases more rapidly (figure 2.10). a pressure equal to that of the boiler, or which stood in a fixed ratio to it
...
Although it may seem laborious, this approach of “we have

a general model for the trend, what are the parameters for this specific case?” is very common in physics, and extremely useful for engineers.
...
This calculation can be done more quickly without calculating

the values of $v_{\mathrm{A}}, v_{\mathrm{B}}$, and $k_{2}$. However, to ensure reaching the correct result, it is safer and easier to quantify $p$ and $v$ (in si) at all stages of the process before starting an integration.
...
Things get more complicated when we compress and expand our fluid « $^{\mathrm{We} \mathrm{have} \mathrm{said} \mathrm{that} \mathrm{at} \mathrm{the} \mathrm{start}}$ of the movement, the pressure equilib-

rapidly (figure 2.11). A complex and critically important phenomenon in rium is established between the boiler

thermodynamics occurs: **the pressure on the wall differs from the** $^{\mathrm{and} \mathrm{the} \mathrm{cylinder}, \mathrm{but} \mathrm{as} \mathrm{the} \mathrm{speed} \mathrm{of} \mathrm{the}}$ piston increases, the latter, so to speak,

**“average pressure” inside the fluid**. escapes ahead of the steam without giving it time to establish this equilibrium, and the pressure in the cylinder
...
From a physics standpoint, the phenomenon of irreversibility is fascinat- « $^{\mathrm{Where} \mathrm{does} \mathrm{irreversibility} \mathrm{come}}$ from? It does not come from Newton’s

ing. Indeed, we start from collisions of molecules, a completely reversible laws. If we claim that the behavior of everything is ultimately to be under-

phenomenon, to create an irreversible process: one that only goes in one stood in terms of the laws of physics,

direction! In order to bring the gas back to the state it was in before being and if it also turns out that all the equations have the fantastic property that if

abruptly compressed, we are forced to transfer heat away from it. It is we put $t=-t$ we have another solution,

surprising that without going against Newton’s laws, we have created a then every phenomenon is reversible. How then does it come about in na-

situation where *we cannot go back by “doing the opposite”*. Are there other ture on a large scale that things are not

:::{math}
reversible? »
:::

irreversible processes? Can we quantify irreversibility? We will attempt to

answer these questions in chapters 7 (*the second law*) and 8 (*entropy*). Richard Feynman, 1963 [[30](#ref-30), [35](#ref-35)] *The Feynman Lectures on Physics*
...
The second problem was more serious and more difficult to solve: how to increase efficiency? It By increasing the transmitted force, the structural was not just a fni ancial question: the first steam- limits of the engine mechanics are quickly reached. powered transatlantic ship, the *SS Savannah*, was On the other hand, increasing the piston travel
...
fni ally $W_{\mathrm{cyl}. \mathrm{A}}= \frac{k_{1}}{0.3} (V_{\mathrm{A}}^{-}_{2}^{0.3}- V_{\mathrm{A}}^{-}_{1}^{0.3}) = +160 J$.
